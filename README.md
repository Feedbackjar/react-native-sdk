...
This is unverified — anyone can type any email. It's stored per-device, so votes/comments still key on this device's anonymous id, not this identity.
...
| `setIdentity({ userId, email, signature, timestamp, name?, firstName?, lastName?, avatar? }): Promise<void>` | Set a **verified**, org-signed identity — see [Verified identity](#verified-identity-signed-in-users). |
...
interface FeedbackIdentity {
  name: string | null;
  email: string | null;
  userId: string | null;       // set only via a verified setIdentity() call
  signature: string | null;
  timestamp: number | null;    // milliseconds since epoch — the value your backend signed
