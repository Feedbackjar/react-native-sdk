...
const USER_ID_STORAGE_KEY = 'com.feedbackjar.sdk.identity.userId';
const SIGNATURE_STORAGE_KEY = 'com.feedbackjar.sdk.identity.signature';
const TIMESTAMP_STORAGE_KEY = 'com.feedbackjar.sdk.identity.timestamp';
const FIRST_NAME_STORAGE_KEY = 'com.feedbackjar.sdk.identity.firstName';
const LAST_NAME_STORAGE_KEY = 'com.feedbackjar.sdk.identity.lastName';
const AVATAR_STORAGE_KEY = 'com.feedbackjar.sdk.identity.avatar';
...
class FeedbackJarClass {
  ...
  /**
   * Remember a submitter's identity — plain `name`/`email` (unverified), or an
   * org-signed `userId`/`email`/`signature`/`timestamp` payload (verified).
   * Omit a field to leave it unchanged; use `clearIdentity()` to remove
   * everything.
   *
   * **Unverified** (just `name`/`email`): anyone can type any email. It's
   * synced to the server against this device's anonymous id (best-effort), so
   * guest votes/comments show the right name and can be reconciled if the user
   * later signs into the web portal with that email.
   *
   ...
   * (`HMAC-SHA256(orgSecretKey, "${userId}:${email}:${timestamp}")`; the org
   * secret key is in the FeedbackJar dashboard). Once set, `vote`,
   * `addComment`, `submit`, and `hasVoted` all attach to this real user
   * instead of the device's anonymous id — on every device, immediately. The
   * first vote/comment after setting it also folds in anything this device
   * already did anonymously, so nothing doubles up. `timestamp` must be the
   * exact millisecond value your backend signed (never a fresh client-side
  ...
  async setIdentity(identity: {
    ...
    await Promise.all([
      ...
      identity.userId != null ? setStoredItem(USER_ID_STORAGE_KEY, identity.userId) : Promise.resolve(),
      identity.signature != null
        ? setStoredItem(SIGNATURE_STORAGE_KEY, identity.signature)
        : Promise.resolve(),
      identity.timestamp != null
        ? setStoredItem(TIMESTAMP_STORAGE_KEY, String(identity.timestamp))
        : Promise.resolve(),
      identity.firstName != null
        ? setStoredItem(FIRST_NAME_STORAGE_KEY, identity.firstName)
        : Promise.resolve(),
      identity.lastName != null
        ? setStoredItem(LAST_NAME_STORAGE_KEY, identity.lastName)
        : Promise.resolve(),
      identity.avatar != null ? setStoredItem(AVATAR_STORAGE_KEY, identity.avatar) : Promise.resolve(),
    ]);
  ...
  async getIdentity(): Promise<FeedbackIdentity> {
    const [name, email, userId, signature, timestampRaw, firstName, lastName, avatar] =
      await Promise.all([
        getStoredItem(NAME_STORAGE_KEY),
        getStoredItem(EMAIL_STORAGE_KEY),
        getStoredItem(USER_ID_STORAGE_KEY),
        getStoredItem(SIGNATURE_STORAGE_KEY),
        getStoredItem(TIMESTAMP_STORAGE_KEY),
        getStoredItem(FIRST_NAME_STORAGE_KEY),
        getStoredItem(LAST_NAME_STORAGE_KEY),
        getStoredItem(AVATAR_STORAGE_KEY),
      ]);
    const timestamp = timestampRaw != null ? Number(timestampRaw) : null;
    return {
      name,
      email,
      userId,
  ...
  async clearIdentity(): Promise<void> {
    await Promise.all([
      removeStoredItem(NAME_STORAGE_KEY),
      removeStoredItem(EMAIL_STORAGE_KEY),
      removeStoredItem(USER_ID_STORAGE_KEY),
      removeStoredItem(SIGNATURE_STORAGE_KEY),
      removeStoredItem(TIMESTAMP_STORAGE_KEY),
      removeStoredItem(FIRST_NAME_STORAGE_KEY),
      removeStoredItem(LAST_NAME_STORAGE_KEY),
      removeStoredItem(AVATAR_STORAGE_KEY),
    ]);
  }

  /**
   * The stored identity as a wire-ready `SignedIdentity`, or null when it
   * isn't verified (no signature) or has aged past `IDENTITY_MAX_AGE_MS` —
   * callers then fall back to the anonymous id, same as if `setIdentity` was
   * never called with a signature.
   */
  private async verifiedIdentity(): Promise<SignedIdentity | null> {
    const identity = await this.getIdentity();
    if (!identity.userId || !identity.email || !identity.signature || identity.timestamp == null) {
      return null;
    }

    const ageMs = Date.now() - identity.timestamp;
  ...
  private async _submit(
    ...
    return submitFeedback(
      this.widgetId,
      {
        ...
        metadata,
        identity: await this.verifiedIdentity(),
      },
      getNativeAppId(),
    );
  }
  ...
  async getPost(postId: string): Promise<FeedbackJarResult<FeedbackPost>> {
    ...
    return apiGetPost(this.widgetId, postId, getNativeAppId(), await getAnonId(), await this.verifiedIdentity());
  }
  ...
  async vote(postId: string): Promise<FeedbackJarResult<VoteState>> {
    ...
    return apiVoteOnPost(this.widgetId!, postId, getNativeAppId(), await getAnonId(), await this.verifiedIdentity());
  }
  ...
  async unvote(postId: string): Promise<FeedbackJarResult<VoteState>> {
    const notReady = this.requireWidgetId();
    if (notReady) return notReady;
    return apiUnvotePost(this.widgetId!, postId, getNativeAppId(), await getAnonId(), await this.verifiedIdentity());
  }
  ...
  async getVoteState(postId: string): Promise<FeedbackJarResult<VoteState>> {
    const notReady = this.requireWidgetId();
    if (notReady) return notReady;
    return apiGetVoteState(this.widgetId!, postId, getNativeAppId(), await getAnonId(), await this.verifiedIdentity());
  }
